...
export function DiagnosticsPage() {
  ...
  // Build per-project lookup from summary billing result (for file-based validation cards)
  const appBillingByProject = useMemo(() => {
    const lookup = new Map<string, AppBillingData>();
    if (!summaryBillingResult) return lookup;
    for (const company of summaryBillingResult.companies) {
  ...
  const isLoading = ratesLoading || companiesLoading;
