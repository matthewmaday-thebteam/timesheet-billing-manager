...
// Helper to get ID count for a resource
function getIdCount(resource: Resource | ResourceWithGrouping): number {
  if (hasGroupingInfo(resource) && resource.all_system_ids.length > 0) {
    return resource.all_system_ids.length;
  }
  if (resource.associations && resource.associations.length > 0) {
    return resource.associations.length;
  }
  return 1; // Default to 1 (the external_label)
}
        ...
        <table className="w-full">
          ...
          <tbody className="divide-y divide-vercel-gray-100">
            {resources.map((resource) => {
              ...
              const idCount = getIdCount(resource);
              ...
              return (
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    {incomplete ? (
                      <span className="inline-flex items-center px-2 py-0.5 rounded-md text-xs font-medium bg-bteam-brand text-white border border-bteam-brand">
