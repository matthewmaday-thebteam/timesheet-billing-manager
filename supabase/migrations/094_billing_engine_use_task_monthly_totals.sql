-- ============================================================================
-- Migration 094: Billing Engine — Read from task_monthly_totals
-- ============================================================================
-- Purpose:
--   Update recalculate_project_month() and compare_summary_vs_recomputed()
--   to read pre-computed rounding values from task_monthly_totals instead of
--   computing them on-the-fly from timesheet_daily_rollups.
--
--   The rounding_mode setting (entry vs task) determines which pre-computed
--   column is used:
...
--
-- Impact:
--   - For all existing data (rounding_mode='task'), results are IDENTICAL
--     to the previous billing engine — no financial impact.
--   - Future months set to rounding_mode='entry' will use entry-level rounding.
...
-- ============================================================================
-- PART 1: Update recalculate_project_month()
-- ============================================================================
-- Changes:
--   DECLARE: Add v_rounding_mode TEXT
--   STEP 1: Also fetch effective_rounding_mode
--   STEP 3: Read from task_monthly_totals instead of timesheet_daily_rollups
...
BEGIN
    ...
    -- populate_task_monthly_totals in migration 093), so we match on
    -- p_project_id directly — no need to use v_project_ids array.
    ...
    WHERE tmt.project_id = p_project_id
...
COMMENT ON FUNCTION recalculate_project_month(UUID, DATE) IS
    ...
    'v2.0-tmt: Reads pre-computed values from task_monthly_totals. '
...
-- ============================================================================
-- PART 2: Update compare_summary_vs_recomputed()
-- ============================================================================
-- Changes:
--   - Add r_rounding_mode variable
--   - Fetch effective_rounding_mode alongside effective_rounding
--   - Replace task rounding CTE with task_monthly_totals read + mode switch
...
BEGIN
    ...
    LOOP
        ...
        -- Recompute from task_monthly_totals (matching recalculate_project_month v2.0-tmt)
        -- Uses canonical project_id directly since task_monthly_totals stores canonical UUIDs
        SELECT
            COALESCE(SUM(tmt.actual_minutes), 0)::INTEGER,
        ...
        FROM task_monthly_totals tmt
        WHERE tmt.project_id = v_rec.project_id
...
COMMENT ON FUNCTION compare_summary_vs_recomputed(DATE) IS
    ...
    'project_monthly_summary. v2.0-tmt: reads from task_monthly_totals with rounding_mode switch. '
    'Returns all rows with a has_discrepancy flag. '
...
BEGIN
    ...
    RAISE NOTICE '  - recalculate_project_month() updated to v2.0-tmt';
    RAISE NOTICE '    STEP 1: Now fetches effective_rounding_mode';
    RAISE NOTICE '    STEP 3: Now reads from task_monthly_totals';
    RAISE NOTICE '    Uses rounded_entry_minutes for mode=entry, rounded_task_minutes for mode=task';
    RAISE NOTICE '  - compare_summary_vs_recomputed() updated to match';
    RAISE NOTICE '  - No table changes, no frontend changes';
    RAISE NOTICE '';
    RAISE NOTICE '  VERIFY: Run compare_summary_vs_recomputed for pre-April months:';
    RAISE NOTICE '    SELECT * FROM compare_summary_vs_recomputed(''2026-03-01'') WHERE has_discrepancy;';
    RAISE NOTICE '    -- Should return zero rows (rounding_mode=task matches existing billing engine)';
END $$;
