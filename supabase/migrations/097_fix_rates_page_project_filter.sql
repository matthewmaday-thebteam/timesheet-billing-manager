-- Fix: get_all_project_rates_for_month() was excluding "unassociated" projects
-- (31 out of 54), breaking the Rates page and milestone adjustments.
-- The WHERE clause should exclude only "member" projects (to prevent double-counting).
-- Also hardcodes all projects to rounding_mode = 'task' until explicitly changed.

-- First, ensure all rounding_mode values are 'task'
UPDATE project_monthly_rounding SET rounding_mode = 'task' WHERE rounding_mode != 'task';
